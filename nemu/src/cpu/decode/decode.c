...
make_DHelper(J) {
  ...
  if(id_dest->width==1)//宽度为1时相对地址相加以后进位要丢弃
  {
    ...
    printf("tmp_eip=%08X\n",tmp_eip);
    printf("simm=%08X\n",id_dest->simm);
    unsigned char temp = (char)(id_dest->simm) + (char)(*eip);
    printf("temp=%08X\n",temp);
...
{
  if (op->type == OP_TYPE_REG) { rtl_sr(op->reg, width, src);}
  else if (op->type == OP_TYPE_MEM) { rtl_sm(&op->addr, width, src); }
  else { assert(0); }
}

void get_mr_value(rtlreg_t* dest,Operand* op)//get mem reg imm value
{
  if(op->type==OP_TYPE_REG)
  {
    rtl_lr(dest,op->reg,op->width);
  }
